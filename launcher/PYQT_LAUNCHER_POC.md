# Qt C++ Launcher + 独立 PyQt 子进程架构 POC 调研报告
...
- **每个功能页面**用 **独立 PyQt6 进程**，通过按键触发启动，退出后回到 launcher
...
graph TD
    ...
    C -->|QProcess 启动| D[python3 pyqt_page.py<br/>QT_QPA_PLATFORM=linuxfb:fb=/dev/fb-spi]
...
| 启动 PyQt 前 `timer->stop() + window.hide()` | linuxfb 无合成器，两个进程抢 fb 会互相覆盖；必须串行 |
| PyQt 退出后 `showFullScreen() + repaint()` | 恢复主界面，避免脏 fb |
| PyQt 侧 30 秒自动退出兜底 | 防止 gpiomon 失效时用户按不出去 |
...
|---|---|---|---|
| 左上 | A | **17** | qt-hello 主进程监听 → 启动 PyQt |
| 右上 | B | 22 | （未用）|
| 右下 | C | 24 | （未用）|
| 左下 | D | **23** | PyQt 页面监听 → 退出 |
...
| 文件 | 作用 |
|---|---|
| [main.cpp](./main.cpp) | Qt C++ 主进程：时间显示 + gpiomon 监听 + QProcess 启动 PyQt + fb 接力 |
| [pyqt_page.py](./pyqt_page.py) | 独立 PyQt6 全屏页面：计数器、分阶段启动耗时日志、退出键监听 |
...
- Qt6 C++（已装，系统包）
- `python3-pyqt6`（通过 `apt install python3-pyqt6` 安装，版本 6.8.2）
...
| Python 入口 → PyQt6 import 完成 | +36 ms | 37 ms |
...
- PyQt 页面 50ms 定时器（20 fps）刷新 counter，SPI 屏显示流畅无撕裂
- qt-hello 主进程完全让出 fb，画面无冲突
- PyQt 退出后 qt-hello 主界面 `repaint()` 立即恢复
...
[qt-hello][<ms>] request -> launch PyQt
[qt-hello][<ms>] ui paused +Xms
[qt-hello][<ms>] QProcess::start() returned +Xms since request
[qt-hello][<ms>] PyQt 'started' signal after Xms (fork+exec)
[qt-hello][<ms>] PyQt finished code=C status=S total=Tms
```

**Python 侧**（stdout，通过 `QProcess::ForwardedChannels` 进入 journal）：
```
[pyqt_page][+  X.Xms] python entry
[pyqt_page][+  X.Xms] PyQt6 import done
[pyqt_page][+  X.Xms] QApplication created
[pyqt_page][+  X.Xms] widget constructed
[pyqt_page][+  X.Xms] first paintEvent
[pyqt_page][+  X.Xms] showFullScreen returned
[pyqt_page] boot breakdown: <每阶段绝对/增量耗时>
```
...
| 3 | PyQt 冷启动受 OS 文件缓存影响 | 首次启动可能 > 200 ms，热缓存 ~85 ms；不影响可用性 |
...
3. **首个真实页面**：把 `run_blockly.py` 按方案 B 重写为 PyQt 版本（UI 用 Qt，业务 `GracefulBlocklyService` / `ProgramRunner` 保留）
