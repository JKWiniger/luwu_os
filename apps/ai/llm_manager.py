    ...
    def __init__(self, config, tool_definitions=None, role_config=None, lang="cn"):
        ...
        if self.lang == "en":
            ...
            self._tts_constraint = ("回复中不要使用*、表情符号、特殊符号，只用纯文本。"
                                   ...
                                   "极其重要：当用户要求机器人执行任何物理动作（前进、后退、左移、右移、转向、执行动作等）时，"
                                   "你必须立即调用对应的工具函数（如xgo_move、xgo_turn、xgo_action等）来实际执行，"
                                   "绝对不能只用文字说'好的我会做'或'我去执行'——说了但不调用工具等于没做。"
                                   "当你调用工具/函数时，不要在调用前后输出任何文字，只返回函数调用本身。")
